...
function addActionsForHtmlUI(){
   ...
   document.getElementById('difficultySlide').addEventListener('mouseup', function() {speed=this.value/200;});
...
function tick(){
   if(gameStarted == true){
      ...
      // score.innerHTML = "Score: " + g_seconds;
...
function updateAnimationAngle(){
   ...
   if(gameStarted){
      for(var i = 0; i < len; i++) {
         g_shapesList[i].position[0]=g_shapesList[i].position[0]-speed;
...
function renderAllShapes(){
   ...
   for(var i = 0; i < len; i++) {
      ...
      if(point.collide(g_shapesList[i])){
         ...
         score.innerHTML = "Score: " + scoreValue;
         break;
      }else if(point.position[0] > g_shapesList[i].position[0] && g_shapesList[i].top && gameStarted == true){
         scoreValue += 1;
         score.innerHTML = "Score: " + scoreValue/10;
      }
...
function restartGame(){
   ...
   pipe_x=-0.4;
   birdPosition = [-.9, -0.02];
   if(scoreValue/10 > highScoreValue) {
      highScoreValue = scoreValue/10;
      highScore.innerHTML = "High Score: " + highScoreValue;
   }
   scoreValue = 0;
